...
export async function onRequestPost({ request, env }) {
  try {
    ...
    await env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?, 'owner', ?, 'active')
    `).bind(fullName, email, username, passwordHash, companyTitle).run();

    const user = await env.DB.prepare('SELECT id FROM users WHERE email = ? LIMIT 1').bind(email).first();
    if (!user) return json({ error: 'Owner was created, but user lookup failed.' }, 500);
    ...
    return json({ ok: true, redirect: '/owner-dashboard.html' }, 200, {
      'Set-Cookie': cookie('jaxtri_session', session.id, session.maxAge),
    });
  } catch (error) {
    return json({ error: error?.message || 'Setup failed inside Cloudflare Function.' }, 500);
  }
}
