...
CREATE TABLE IF NOT EXISTS affiliate_uploads (
  ...
  purpose TEXT NOT NULL DEFAULT 'payout_proof',
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_affiliate_uploads_user_id ON affiliate_uploads(user_id);
CREATE INDEX IF NOT EXISTS idx_affiliate_uploads_purpose ON affiliate_uploads(purpose);
CREATE INDEX IF NOT EXISTS idx_affiliate_uploads_created ON affiliate_uploads(created_at);
...
CREATE TABLE IF NOT EXISTS payout_requests (
  ...
  FOREIGN KEY(proof_upload_id) REFERENCES affiliate_uploads(id) ON DELETE SET NULL
);
