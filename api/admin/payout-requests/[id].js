...
export async function onRequestPatch({ request, env, params }) {
  try {
    ...
    const proofUploadId = Number(body.proof_upload_id || 0);
    const finalProofUploadId = Number.isFinite(proofUploadId) && proofUploadId > 0 ? proofUploadId : null;

    if (nextStatus === 'paid' && !transactionId && !proofUrl && !finalProofUploadId) {
      return json({ error: 'Add a PayPal/reference ID or proof screenshot/link before marking paid.' }, 400);
    }
    ...
    if (nextStatus === 'paid') {
      await env.DB.prepare(`
        ...
        WHERE id = ?
      `).bind(transactionId, finalProofUploadId, proofUrl, adminNote, actor.id, actor.id, id).run();
    ...
    try {
      ...
      `).bind(actor.id, payout.affiliate_user_id, JSON.stringify({ payout_request_id: id, status: nextStatus, transaction_id: transactionId, proof_upload_id: finalProofUploadId, proof_url: proofUrl })).run();
    } catch {}
