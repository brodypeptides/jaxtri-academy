...
async function requireSetup(env) {
  const missing = [];
  for (const table of ['payout_requests', 'payout_request_sales', 'affiliate_uploads']) {
  ...
  try {
    ...
    let sql = `
      SELECT
        ...
        up.file_name AS proof_file_name,
        up.content_type AS proof_content_type,
      ...
      LEFT JOIN affiliate_uploads up ON up.id = pr.proof_upload_id
